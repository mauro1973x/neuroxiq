import { Link, useNavigate, useLocation } from "react-router-dom";
import logo from '@/assets/logo.png';

const Footer = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const handleTestClick = (e: React.MouseEvent<HTMLAnchorElement>, anchorId: string) => {
    e.preventDefault();
    
    const scrollToSection = () => {
      const element = document.getElementById(anchorId);
      if (element) {
        element.scrollIntoView({ behavior: "smooth", block: "start" });
        window.history.replaceState(null, "", `/#${anchorId}`);
      }
    };

    // If already on home page, just scroll
    if (location.pathname === "/") {
      scrollToSection();
    } else {
      // Navigate to home first, then scroll after a short delay
      navigate("/");
      setTimeout(scrollToSection, 100);
    }
  };

  return (
    <footer className="border-t border-border bg-muted/30">
      <div className="container py-10 md:py-12 px-4">
        {/* Mobile: Stack everything, Desktop: Grid */}
        <div className="flex flex-col gap-8 md:grid md:grid-cols-4 md:gap-8">
          {/* Brand */}
          <div className="md:col-span-1">
            <Link to="/" className="flex items-center gap-2 mb-4">
              <img src={logo} alt="NEUROX Logo" className="h-8 w-8 md:h-9 md:w-9 object-contain" />
              <span className="font-display text-lg md:text-xl font-bold text-foreground">NEUROX</span>
            </Link>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Descubra mais sobre você através de testes científicos e personalizados.
            </p>
          </div>

          {/* Links Grid - 2 columns on mobile, full on desktop */}
          <div className="grid grid-cols-2 gap-6 md:contents">
            {/* Testes */}
            <div>
              <h4 className="font-display font-semibold mb-3 md:mb-4 text-sm md:text-base">Testes</h4>
              <ul className="space-y-2.5 md:space-y-2">
                <li>
                  <a
                    href="/#teste-qi"
                    onClick={(e) => handleTestClick(e, "teste-qi")}
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 cursor-pointer"
                  >
                    Teste de QI
                  </a>
                </li>
                <li>
                  <a
                    href="/#teste-personalidade"
                    onClick={(e) => handleTestClick(e, "teste-personalidade")}
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 cursor-pointer"
                  >
                    Personalidade
                  </a>
                </li>
                <li>
                  <a
                    href="/#teste-carreira"
                    onClick={(e) => handleTestClick(e, "teste-carreira")}
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 cursor-pointer"
                  >
                    Carreira
                  </a>
                </li>
                <li>
                  <a
                    href="/#teste-inteligencia-emocional"
                    onClick={(e) => handleTestClick(e, "teste-inteligencia-emocional")}
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 cursor-pointer"
                  >
                    Inteligência Emocional
                  </a>
                </li>
                <li>
                  <a
                    href="/#teste-politico"
                    onClick={(e) => handleTestClick(e, "teste-politico")}
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 cursor-pointer"
                  >
                    Orientação Política
                  </a>
                </li>
              </ul>
            </div>

            {/* Contato */}
            <div>
              <h4 className="font-display font-semibold mb-3 md:mb-4 text-sm md:text-base">Contato</h4>
              <ul className="space-y-2.5 md:space-y-2">
                <li>
                  <a 
                    href="mailto:hello.neurox@gmail.com" 
                    className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1 break-all"
                  >
                    hello.neurox@gmail.com
                  </a>
                </li>
                <li>
                  <Link to="/sobre" className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1">
                    Sobre Nós
                  </Link>
                </li>
                <li>
                  <Link to="/precos" className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1">
                    Preços
                  </Link>
                </li>
              </ul>
            </div>
          </div>

          {/* Legal - Full width on mobile */}
          <div className="md:col-span-1">
            <h4 className="font-display font-semibold mb-3 md:mb-4 text-sm md:text-base">Legal</h4>
            <ul className="flex flex-row gap-4 md:flex-col md:gap-0 md:space-y-2">
              <li>
                <Link to="/termos" className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1">
                  Termos de Uso
                </Link>
              </li>
              <li>
                <Link
                  to="/privacidade"
                  className="text-sm text-muted-foreground hover:text-foreground transition-colors block py-1"
                >
                  Privacidade
                </Link>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom section */}
        <div className="border-t border-border mt-8 pt-6 md:pt-8 flex flex-col-reverse md:flex-row justify-between items-center gap-4">
          <p className="text-xs md:text-sm text-muted-foreground text-center md:text-left">
            © {new Date().getFullYear()} NEUROX. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-5">
            {/* Instagram */}
            <a 
              href="https://instagram.com/neuroxiq" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-muted-foreground hover:text-foreground transition-colors p-2 -m-2"
              aria-label="Instagram"
            >
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            </a>
            {/* TikTok */}
            <a 
              href="https://tiktok.com/@neuroxiq" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-muted-foreground hover:text-foreground transition-colors p-2 -m-2"
              aria-label="TikTok"
            >
              <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
              </svg>
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
